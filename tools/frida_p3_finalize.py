#!/usr/bin/env python3
"""Build the P3.2.3.14 summary for the current CAPEsolo attempt.

P3.2.3.14 retains the active-time child observation window, gates cold
...
def detect_behavior_telemetry_gaps(records: list[dict]) -> dict:
    ...
    P3.2.3.14 checks registry handle continuity. A RegSetValue call
...
def build_report(analysis_dir: Path, output_dir: Path | None = None) -> dict:
    ...
    report = {
        "schema": "capesolo-frida-p3-report/1.2.3.14",
        "version": run_version,
        "processor_version": PRODUCT_VERSION,
        ...
        "notes": [
            "P3.2.3.14 retains P3.2/P3.2.1 run scoping and provenance invariants.",
            ...
            "P3.2.3.14 keeps behavior.compact.jsonl as a presentation-only semantic view; behavior.filtered.jsonl remains lossless and unchanged.",
            "P3.2.3.14 reports missing file materialization without synthesizing CopyFile/CreateFile evidence.",
            "P3.2.3.14 retains generic behavior-chain summaries for persistence, stage hand-off, and conservative injection precursors.",
            "P3.2.3.14 separates Frida agent attach, fast child hooks, CAPEMON-observed short-lived children, and full hooks-ready outcomes per PID.",
            ...
            "P3.2.3.14 preserves target_died_during_optional_attach and reports possible instrumentation interference without claiming causation.",
