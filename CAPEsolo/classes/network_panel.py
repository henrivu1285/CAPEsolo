...
class NetworkPanel(wx.Panel, KeyEventHandlerMixin):
    ...
    def InitUI(self):
        ...
        taskCapture = discover_task_capture(self.analysisDir)
        self.pcapPath.SetValue(
            str(taskCapture) if taskCapture else "<task pcapng, or select an external capture>"
        )
        ...
        self.grid = CopyableGrid(self.splitter, 0, 9)
        for col, label in enumerate(
            ("Time", "Kind", "PID", "Process", "Source", "Destination", "Keys", "Seen", "Info")
        ):
    ...
    def ProcessCapture(self, event):
        ...
        try:
            with wx.BusyCursor():
                ...
                sysmonEvents, lineage = load_network_evidence(self.analysisDir)
                self.results = correlate_capture(
                    self.results,
                    sysmonEvents,
                    lineage,
                    clock_sync=load_clock_sync(self.analysisDir),
                )
    ...
    def AddTableData(self):
        ...
        for row, data in enumerate(self.viewRows):
            ...
            self.grid.SetCellValue(row, 2, str(attribution.get("pid") or ""))
            process = Path(str(attribution.get("process") or "")).name
            self.grid.SetCellValue(row, 3, process)
            self.grid.SetCellValue(row, 4, str(data["src"]))
            self.grid.SetCellValue(row, 5, str(data["dst"]))
            self.grid.SetCellValue(row, 6, data.get("keys", ""))
            self.grid.SetCellValue(row, 7, str(repeats) if repeats > 1 else "")
            ...
            self.grid.SetCellValue(row, 8, info[:512])

        self.grid.AutoSizeColumns()
        for col, limit in ((3, ADDR_COL_MAX), (4, ADDR_COL_MAX), (5, ADDR_COL_MAX), (8, INFO_COL_MAX)):
    ...
    def Summarize(self):
        ...
        attribution = self.results.get("attribution") or {}
        flowAttr = attribution.get("flows") or {}
        dnsAttr = attribution.get("dns") or {}
        lines.append(
            "PID map:   flows mapped={mapped}/{total} high={high} medium={medium} "
            "ambiguous={ambiguous} unmapped={unmapped}".format(
                mapped=flowAttr.get("mapped", 0),
                total=flowAttr.get("total", 0),
                high=flowAttr.get("high", 0),
                medium=flowAttr.get("medium", 0),
                ambiguous=flowAttr.get("ambiguous", 0),
                unmapped=flowAttr.get("unmapped", 0),
            )
        )
        lines.append(
            "DNS map:   requester mapped={mapped}/{total} ambiguous={ambiguous} unmapped={unmapped}".format(
                mapped=dnsAttr.get("mapped", 0),
                total=dnsAttr.get("total", 0),
                ambiguous=dnsAttr.get("ambiguous", 0),
                unmapped=dnsAttr.get("unmapped", 0),
            )
        )
        ...
        if 0 <= row < len(self.viewRows):
            ...
            if attribution:
                detail += (
                    ...
                    f"    method: {attribution.get('method')}\n"
                    f"    pid: {attribution.get('pid')}\n"
                    f"    process: {attribution.get('process')}\n"
                    f"    role: {attribution.get('role')}\n"
                    f"    tracked task process: {attribution.get('tracked')}"
                )
