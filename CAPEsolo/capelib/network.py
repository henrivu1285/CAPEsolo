...
CAPEsolo P3.2.3.14 can fetch a task-scoped pcapng from the isolated INetSim VM; a capture
selected manually in the Network panel remains supported. The TLS
...
def ParseDns(payload):
    ...
    query_types = [dns.rdatatype.to_text(question.rdtype) for question in message.question]
    answers = []
...
def ParseHttp(payload):
    ...
    parts = startLine.split(" ", 2)
    if is_response:
        parsed.update(
            version=parts[0].removeprefix("HTTP/") if parts else "",
        ...
        parsed.update(
            ...
            version=parts[2].removeprefix("HTTP/") if len(parts) > 2 else "",
            status_code="",
            reason="",
        )
    return parsed
...
def _DnsEvent(packet, payload, hosts):
    ...
    return {
        ...
        "dns": {
            "query": query,
            "query_type": parsed.get("query_types", [""])[0] if parsed.get("query_types") else "",
            "response": bool(parsed["response"]),
            "rcode": parsed["rcode"],
...
def _HttpEvent(packet, payload):
    ...
    host = next(
        (value for name, value in parsed["headers"].items() if name.lower() == "host"),
        "",
    )
...
def _TlsEvent(packet, payload, keylog, hosts, sessions):
    ...
    return {
        ...
        "tls": {
            ...
            "client_random": parsed["client_random"],
            "has_keys": bool(hasKeys),
            "truncated": bool(parsed["truncated"]),
        },
    }


def _ConnectionKey(src, sport, dst, dport, protocol):
    left, right = (src, sport), (dst, dport)
    return (min(left, right), max(left, right), protocol)


def _FlowRows(flows, hosts, connectionHosts=None):
    """Collapse the per-packet counters into one row per conversation."""
    connectionHosts = connectionHosts or {}
    rows = []
    for key in sorted(flows, key=lambda k: flows[k]["bytes"], reverse=True):
        ...
        exactHosts = connectionHosts.get(
            _ConnectionKey(src, sport, dst, dport, protocol), set()
        )
        exactHost = sorted(exactHosts)[0] if exactHosts else ""
        rows.append(
            {
                "kind": "Flow",
                "time": stats["first"],
                "src": f"{src}:{sport}",
                "dst": f"{dst}:{dport}",
                # HTTP Host/SNI belongs to this socket. Falling back to a global
                # DNS answer for an INetSim IP can assign one of many unrelated
                # names to every flow sharing 192.168.56.2.
                "host": exactHost or sorted(hosts.get(dst, [""]))[0],
                "info": (
                    f'{name}, {stats["packets"]} packet(s), '
                    f'{stats["bytes"]} payload byte(s)'
                ),
                "keys": "",
                "flow_id": f"{name.lower()}:{src}:{sport}>{dst}:{dport}",
                "protocol": name.lower(),
                "src_ip": src,
                "src_port": sport,
                "dst_ip": dst,
                ...
                "detail": _FormatDetail(
                    [
                        ("Protocol", name),
                        ("Source", f"{src}:{sport}"),
                        ("Destination", f"{exactHost or _Host(hosts, dst)}:{dport}"),
                        ("Packets", stats["packets"]),
                        ("Payload bytes", stats["bytes"]),
                        ("First seen", FormatTime(stats["first"])),
                        ("Last seen", FormatTime(stats["last"])),
                    ]
                ),
            }
        )
...
def CollapseEvents(events):
    ...
    for source in events:
        ...
        key = event.pop("collapse", None)
        if key is None:
            key = (
                event.get("kind"), event.get("src_ip"), event.get("src_port"),
                event.get("dst_ip"), event.get("dst_port"), event.get("info"),
            )
        first = collapsed.get(key)
        if first is None:
            event["repeats"] = 1
            event["first"] = event.get("time")
            event["last"] = event.get("time")
            collapsed[key] = event
            continue

        first["repeats"] += 1
        first["last"] = event["time"]
        # A later event can carry a better PID mapping after the final Sysmon
        # drain. Preserve all distinct attributions instead of hiding that fact
        # behind the first display row.
        attrs = first.setdefault("attributions", [])
        for candidate in (first.get("attribution"), event.get("attribution")):
            if not isinstance(candidate, dict):
                continue
            identity = (
                candidate.get("status"), candidate.get("pid"),
                candidate.get("process_guid"), candidate.get("method"),
            )
            known = {
                (item.get("status"), item.get("pid"), item.get("process_guid"), item.get("method"))
                for item in attrs if isinstance(item, dict)
            }
            if identity not in known:
                attrs.append(deepcopy(candidate))

    rows = list(collapsed.values())
    for row in rows:
        if row["repeats"] > 1:
            last = FormatTime(row.get("last", row["time"]))
            note = f'Seen {row["repeats"]} times, last at {last}.'
            row["detail"] = (
                f'{row.get("detail")}\n\n{note}' if row.get("detail") else note
            )

    return rows


# Compatibility for callers/tests written against P3.2.3.9's private name.
_CollapseEvents = CollapseEvents
...
def NetworkData(analysisDir, pcapPath, maxPackets=MAX_PACKETS):
    ...
    for timestamp, frame in packets:
        ...
        eventHost = str(event.get("host") or "").strip().rstrip(".").lower()
        if eventHost:
            connectionHosts.setdefault(
                _ConnectionKey(
                    packet["src"], packet["sport"], packet["dst"],
                    packet["dport"], packet["protocol"],
                ),
                set(),
            ).add(eventHost)
        counts[event["kind"]] += 1
    ...
    return {
        "pcap": str(pcapPath),
        "keylog": keylog,
        # Keep every protocol occurrence for PID attribution and machine-readable
        # accounting. The UI consumes display_events, where semantically identical
        # retries are collapsed only after each occurrence has been attributed.
        "events": events,
        "display_events": CollapseEvents(events),
        "flows": _FlowRows(flows, hosts, connectionHosts),
