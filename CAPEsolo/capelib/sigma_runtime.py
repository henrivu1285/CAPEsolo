...
def normalize_events(results: dict, clean_rows: Optional[list[dict]] = None) -> tuple[list[dict], dict]:
    ...
    for process, call in _iter_calls(results, clean_rows):
        ...
        elif low.startswith(("copyfile", "createfile", "ntcreatefile", "writefile", "ntwritefile")):
...
def _evaluate(node: dict, fields: dict, negated: bool = False) -> tuple[bool, set[str]]:
    ...
    if op == "field":
        ...
        matched = _value_match(spec, value if present else None, fields)
        positive = {name} if matched and not negated and spec.get("kind") != "null" else set()
        return matched, positive
    if op == "not":
        matched, _ = _evaluate(node.get("arg") or {}, fields, not negated)
        return not matched, set()
    if op in {"and", "or"}:
        evaluated = [_evaluate(item, fields, negated) for item in node.get("args") or []]
        if op == "and":
            matched = bool(evaluated) and all(item[0] for item in evaluated)
            positive = set().union(*(item[1] for item in evaluated)) if matched else set()
        else:
            successful = [item for item in evaluated if item[0]]
            matched = bool(successful)
            positive = set().union(*(item[1] for item in successful)) if successful else set()
        return matched, positive
    return False, set()
...
def evaluate_sigma(results: dict, analysis_path: Optional[Path] = None,
    ...
    events, telemetry = normalize_events(results, clean_rows)
    ...
    for rule in pack.get("rules") or []:
        ...
        for event in by_category.get(category) or []:
            matched, positive_fields = _evaluate(rule.get("condition") or {}, event.get("fields") or {})
